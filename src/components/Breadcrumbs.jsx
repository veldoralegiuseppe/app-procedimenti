    ...
    function Links(){
        ...
        return subPath.map((path, index) => (
            <Link key={path} 
                 ...
                 <Typography variant="h6" sx={{fontSize: '1.1rem', fontWeight: '400', color: theme.palette.text.primary}}>{camelCase(path.replace('/',''))}</Typography>
            </Link>
        ))
    }
    ...
    return (
        ...
        <Breadcrumbs aria-label="breadcrumb" separator="›" sx={{'& .MuiBreadcrumbs-separator':{fontSize: '1.5rem', paddingBottom: '0.15rem'} }}>
            <Link 
                ...
                <HomeIcon sx={{ mr: 0.5, color: '#fb8500', width:'1.5rem', height: '1.5rem' }} fontSize="inherit" />
            </Link>
            {Links()}
        </Breadcrumbs>
        </div>
