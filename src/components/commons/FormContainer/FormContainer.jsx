...
const FormContainer = React.memo(
  ...
  (prevProps, nextProps) => {
    ...
    const isSezioniEqual = _.isEqual(prevProps.sezioni, nextProps.sezioni);

    // Controllo del config
    const isConfigEqual = _.isEqual(prevProps.config.renderOverrides, nextProps.config.renderOverrides);

    return isConfigEqual && isSezioniEqual;
  }
);
